        ...
        for word in word_set:
            ...
            if center not in word:
                if debug and word in ['capo', 'capon', 'papa', 'atop', 'upon']:
                    print(f"  {word:<15} [{source_name}] FILTERED: No center letter '{center}'")
                continue
            ...
            if not set(word) <= allowed_letters:
                if debug and word in ['capo', 'capon', 'papa', 'atop', 'upon']:
                    ...
                    print(f"  {word:<15} [{source_name}] FILTERED: Uses forbidden letters {forbidden}")
                continue
                
            # Must be long enough
            if len(word) < 4:
                if debug and word in ['capo', 'capon', 'papa', 'atop', 'upon']:
                    print(f"  {word:<15} [{source_name}] FILTERED: Too short ({len(word)} chars)")
                continue
                
            # Skip rejected words
            if word in rejected_words:
                if debug:
                    print(f"  {word:<15} [{source_name}] FILTERED: In rejected words list")
                continue
            ...
            source_display = "+".join(source_list) if len(source_list) > 1 else source_name
            ...
            if debug:
                ...
                print(f"  {word:<15} [{source_display}] ACCEPTED: {points} pts, {confidence}% confidence{pangram_note}")
