...
def load_webster_dictionary():
    ...
    try:
        ...
        with open(
            "/usr / share / dict / american - english", "r", encoding="utf - 8"
        ) as f:
            words = []
            for line in f:
            ...
            print("Primary dictionary loaded: {len(words)} words")
            return words
    except FileNotFoundError:
        print("Webster's dictionary not found, using fallback")
        return []
...
def load_aspell_dictionary():
    ...
    try:
        ...
        print("Aspell dictionary loaded: {len(words)} words")
        return words
...
def load_scrabble_dictionary():
    ...
    try:
        with open("scrabble_words.txt", "r", encoding="utf - 8") as f:
            ...
            print("Scrabble dictionary loaded: {len(words)} words")
            return words
    except FileNotFoundError:
    ...
    try:
        with open("comprehensive_words.txt", "r", encoding="utf - 8") as f:
            ...
            for line in f:
                ...
                # Only apply basic validity check to processed word, not intelligent
                # filtering
                if not word_filtering.is_likely_nyt_word(processed_word):
                    continue
            ...
            print("Comprehensive dictionary loaded: {len(words)} words")
            return words
    except FileNotFoundError:
