...
class NLTKProperNounFilter:
    ...
    def setup_nltk_data(self):
        ...
        required_data = ["punkt", "averaged_perceptron_tagger", "names"]

        for data in required_data:
            try:
                nltk.data.find("tokenizers/{data}")
            except LookupError:
                try:
                    nltk.data.find("taggers/{data}")
                except LookupError:
                    try:
                        nltk.data.find("corpora/{data}")
                    except LookupError:
    ...
    def load_name_sets(self):
        ...
        try:
            ...
            print("Loaded {len(self.all_names)} names from NLTK corpus")
        except Exception as e:
            print("Warning: Could not load NLTK names corpus: {e}")
            self.all_names = set()
