...
def test_unified_solver():
    ...
    try:
        from src.spelling_bee_solver.unified_solver import UnifiedSpellingBeeSolver, SolverMode
        ...
        if solver.dictionary_sources:
            ...
            if os.path.exists(first_dict_path):
                ...
                print(f"✓ Dictionary loading successful: {len(words)} words from {first_dict_name}")
        
        # Test word validation
        valid = solver.is_valid_word_basic("count", "nacuotp", "n")
        print(f"✓ Word validation successful: 'count' is {'valid' if valid else 'invalid'}")
...
def test_word_filtering():
    ...
    try:
        from src.spelling_bee_solver.word_filtering import is_likely_nyt_rejected, filter_words, get_word_confidence
...
def test_gpu_components():
    ...
    try:
        ...
        try:
            ...
            print(f"✓ GPU comprehensive filtering: {len(test_words)} -> {len(filtered)} words")
        ...
        try:
            ...
            print(f"✓ CUDA tokenization: {len(test_texts)} texts -> {len(tokens)} token lists")
        ...
        try:
            ...
            print(f"✓ GPU solver validation: 'count' is {'valid' if valid else 'invalid'}")
