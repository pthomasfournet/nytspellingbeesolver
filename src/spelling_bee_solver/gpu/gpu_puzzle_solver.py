...
class GPUPuzzleSolver:
    ...
    def filter_words_fast(self, words: List[str]) -> List[str]:
        ...
        logger.info("Fast filtering: removed %d/%d words in %.2fs", 
                   removed, len(words), elapsed)
        
        return nyt_filtered
    
    def solve_puzzle_multi_tier(self, letters: str, required_letter: str = None) -> List[str]:
        """
        ...
        for phase_num, (dict_name, dict_path) in enumerate(self.dictionary_sources, 1):
            ...
            logger.info("Found %d candidate words from %s", len(candidate_words), dict_name)
            ...
            if candidate_words:
                ...
                logger.info("Phase %d found %d valid words", phase_num, len(valid_words))
    ...
    def print_results(self, words: List[str], letters: str, required_letter: str):
        ...
        print(f"\n{'='*60}")
        print(f"SPELLING BEE SOLVER RESULTS")
        print(f"{'='*60}")
        ...
        if self.stats["total_time"] > 0:
            ...
            print(f"  Overall: {self.stats['total_words_processed']} words in {self.stats['total_time']:.2f}s ({rate:.1f} words/sec)")
...
def main():
    ...
    parser.add_argument('letters', nargs='?', default='nacuot', 
