...
def is_likely_nyt_rejected(word):
    ...
    place_names = {
        'poona', 'naacp', 'patton', 'pocono', 'tucson', 'yukon', 'pontiac',
        'penn', 'canton', 'newton', 'sutton', 'patna', 'napa', 'poona'
    }
    
    if word_lower in place_names:
        return True
    
    # Check for obvious abbreviations (words with periods or common abbreviation patterns)
...
def debug_solve():
    ...
    print(f"Center: {center}, Allowed: {allowed_letters}")
    print(f"Dictionary size: {len(system_dict)}")
    print(f"Rejected words: {len(rejected_words)}")
    ...
    for word in system_dict:
        ...
        if debug_this:
            print(f"\nDEBUG {word}:")
            print(f"  In rejected list: {word in rejected_words}")
        
        # Skip rejected words
        if word in rejected_words:
            if debug_this: print(f"  FILTERED: In rejected list")
            continue
        
        # Must contain center letter
        if center not in word:
            if debug_this: print(f"  FILTERED: No center letter '{center}'")
            continue
        
        # Must only use allowed letters
        if not set(word) <= allowed_letters:
            if debug_this: print(f"  FILTERED: Uses forbidden letters: {set(word) - allowed_letters}")
            continue
        
        # Length check
        if len(word) < 4:
            if debug_this: print(f"  FILTERED: Too short ({len(word)} chars)")
            continue
        
        # NYT rejection filter
        if is_likely_nyt_rejected(word):
            if debug_this: print(f"  FILTERED: Likely rejected by NYT")
            continue
        
        # NYT word filter  
        if not is_likely_nyt_word(word):
            if debug_this: print(f"  FILTERED: Not likely NYT word")
            continue
            
        if debug_this: print(f"  INCLUDED! ✓")
